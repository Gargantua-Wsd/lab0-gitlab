...
**其一，它是协作的基础设施。** 上面两篇材料回答的其实是同一个问题：很多人同时改同一份代码时怎么让事情不乱。Git 用分支做隔离，用合并做汇合，再用 commit message 规范让汇合时人能看懂发生了什么。这次我在 `main` 和 `feature` 上分别改了同一行，merge 立刻报冲突——那一刻才具体感受到，没有这套机制的话，冲突只能靠人对着屏幕一行行看，而且一定会漏。

**其二，它让我敢于改代码。** 不会用 Git 时，我对重构是有心理负担的：万一改完不行又回不去怎么办？所以要么不敢动手，要么先复制一份备份。有了版本控制，任何一个通过测试的状态都是可以随时回去的锚点，试错成本降下来，人才更愿意尝试更好的写法。

**其三，提交历史本身就是文档。** commit message 规范要求我在提交时就把"为什么这么改"写下来，而这个时候恰恰是我最清楚的；等写文档时再回忆，当时的权衡多半已经忘了。

**其四，它是现代开发流程的入口。** 从代码托管、Pull Request 评审，到 CI/CD 根据分支自动触发构建，全都建立在 Git 的分支和提交模型之上。它不是一个独立技能，而是进入现代软件工程实践的准入条件。
...
```bash
git config --global user.name "Gargantua-Wsd"
